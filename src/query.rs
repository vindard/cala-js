...
impl From<cala_ledger::transaction::TransactionByCreatedAtCursor> for CursorToken {
  fn from(cursor: cala_ledger::transaction::TransactionByCreatedAtCursor) -> Self {
    use base64::{engine::general_purpose, Engine as _};
    let json = serde_json::to_string(&cursor).expect("could not serialize token");
    let token: String = general_purpose::STANDARD_NO_PAD.encode(json.as_bytes());
    CursorToken { token }
...
impl TryFrom<CursorToken> for cala_ledger::transaction::TransactionByCreatedAtCursor {
  type Error = napi::Error;
